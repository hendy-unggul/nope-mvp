...
<!-- CSP FIX -->
<meta http-equiv="Content-Security-Policy" content="default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval' https://cdn.jsdelivr.net https://fonts.googleapis.com https://jejak.space; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; connect-src 'self' *.supabase.co wss://*.supabase.co https://jejak.space https://cdn.jsdelivr.net; worker-src 'self' blob:; manifest-src 'self'; img-src 'self' data:; frame-src 'self';">
...
.spill-head{
  ...
  padding:12px 16px;border-bottom:2px solid var(--bd);background:var(--bg);
}
  ...
  <div class="modal-body">
    ...
    <div class="gender-grid">
      <div class="gender-card" data-gender="female">
        ...
        <span class="gender-desc">mau ngobrol sama cowok</span>
      </div>
      <div class="gender-card" data-gender="male">
        <span class="gender-icon">♂️</span>
        <span class="gender-label">COWOK</span>
        <span class="gender-desc">mau ngobrol sama cewek</span>
      </div>
    </div>
    <div class="info-box">
...
<!-- SPILL GENERATOR - URUTAN PENTING! -->
...
const TYPING_SPEED = { min: 18, max: 38, punct: 110, sentence: 230 };
...
async function callChatAPI(userMsg, isFirst = false) {
  ...
  if (msgs) { 
    msgs.appendChild(td); 
    setTimeout(() => td.scrollIntoView({ behavior:'smooth', block:'end' }), 10); 
  }
  
  const historyForAPI = isFirst ? [] : chatHistory.slice(-MAX_HISTORY_LEN).map(m => ({ role: m.role, content: m.content }));
  
  let reply = null;
  let useOffline = false;
  let attempts = 0;
  
  while (attempts <= 2 && !reply) {
    try {
      ...
      console.warn(`[API] Attempt ${attempts+1} failed:`, e.message); 
    }
    
    attempts++;
    
    if (attempts <= 2) {
      setChatStatus(`⏳ Mencoba lagi (${attempts}/2)...`, 'info');
  ...
  if (useOffline || !reply) {
    ...
    if (apiRetryCount >= 2) setChatStatus('⚡ mode offline aktif', 'info', 6000);
  }
  ...
  await typeMessage(bubble, reply, apiRetryCount >= 2);
