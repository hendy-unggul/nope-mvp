...
// v9.1 — FINAL FIX: Semua deteksi jadi hint, bukan return langsung
// Vercel Serverless Function

const PERSONAS = {
  'beby.manis': {
    style: 'manja tapi asik, sering bilang "ih", "yaampun", "serius??"',
    gender: 'female', greetingStyle: 'pasif',
  ...
  'strawberry.shortcake': {
    ...
    gender: 'female', greetingStyle: 'aktif',
    backstory: 'fresh graduate DKV ITB, sekarang freelance graphic designer.',
  ...
  'pretty.sad': {
    ...
    gender: 'female', greetingStyle: 'pasif',
    backstory: 'karyawan di startup edutech sebagai content writer, udah 2 tahun kerja.',
  ...
  'little.fairy': {
    ...
    gender: 'female', greetingStyle: 'random',
    backstory: 'mahasiswi sastra inggris di UI, suka nulis puisi dan cerpen.',
    aktivitas: ['lagi baca novel buat tugas kuliah','lagi nulis cerpen baru','lagi koreksi tugas mahasiswa','lagi di perpus cari referensi'],
  ...
  'cinnamon.girl': {
    ...
    gender: 'female', greetingStyle: 'aktif',
    backstory: 'karyawan fintech sebagai data analyst. kalem tapi kocak kalau udah nyaman.',
  ...
  'sejuta.badai': {
    ...
    gender: 'male', greetingStyle: 'langsung',
    backstory: 'anak teknik mesin yang lagi kerja praktek di pabrik.',
    aktivitas: ['lagi ngetik laporan KP','lagi di bengkel praktikum','lagi ngopi biar ga ngantuk','lagi ngerjain gambar teknik'],
  ...
  'agak.koplak': {
    ...
    gender: 'male', greetingStyle: 'aktif',
  ...
  'chili.padi': {
    ...
    gender: 'male', greetingStyle: 'langsung',
    backstory: 'karyawan perusahaan konsultan, sering lembur tapi tetep sarkas.',
    aktivitas: ['lagi bikin slide presentasi','lagi meeting client','lagi ngecek data excel','lagi istirahat sebentar'],
...
function handleMinimalInput(message) {
  ...
  const isMinimal = (
    msg.length <= 2 ||
    /^(hi+|he+|ha+|ho+|h[aeiou])+$/i.test(msg) ||
  ...
  if (/^h[aie]+$/i.test(msg)) {
    return ['hai juga!','haloo','hai hai, apa kabar?','halo! lagi ngapain?','hai, ada apa?'][Math.floor(Math.random()*5)];
  }
  if (/^(hehe|haha|hihi|wkwk|wk|kwk)+$/i.test(msg)) {
    return ['haha, ada yang lucu?','wkwk ngapain aja lo?','hehe, cerita dong'][Math.floor(Math.random()*3)];
  }
  if (/^[aeiou]+$/i.test(msg) || /^(oh|ah|eh|ih|uh)$/i.test(msg)) {
    return ['hmm, ada apa?','iya?','hm? gue dengerin'][Math.floor(Math.random()*3)];
  }
  return ['hmm?','iya?','ada apa?'][Math.floor(Math.random()*3)];
}
...
function cleanResponse(reply, userName, shortName, characterName, charGender, userGender = null) {
  ...
  c = c.replace(/(\w+)\.\w+/g, '$1');
  if (userGender === 'male')   ['sis','kak','mbak','sister','say'].forEach(w => { c = c.replace(new RegExp(`\\b${w}\\b`,'gi'), call); });
  if (userGender === 'female') ['bang','bro','cuy','abang','brother'].forEach(w => { c = c.replace(new RegExp(`\\b${w}\\b`,'gi'), call); });
  if (userName && userName !== 'user') c = c.replace(new RegExp(`\\b${userName.replace('@','')}\\b`,'gi'), call);
...
function buildSystemPrompt(characterName, persona, validLastMessages, userName, shortNameUser, userGender, interactionCount, isHigh, isLow, customHints = '') {
  ...
  let userCall;
  if (userGender === 'male')        userCall = 'Panggil dia: bang, bro, cuy (JANGAN sis/kak)';
  else if (userGender === 'female') userCall = 'Panggil dia: kak, sis, say (JANGAN bro/bang)';
  else                               userCall = charGender === 'male' ? 'Panggil dia: bro, bang (sesekali)' : 'Panggil dia: kak, sis (sesekali)';

  let distressNote = '';
  if (isHigh) distressNote = '\n\n🔥 DISTRESS TINGGI: User mungkin dalam kondisi krisis. Respons dengan empati hangat dan tanya apakah mereka mau cerita. Jangan kasih solusi dulu.';
  else if (isLow) distressNote = '\n\n😟 DISTRESS RENDAH: User terlihat sedih/lelah. Beri empati sebagai teman, tapi jadi diri sendiri.';
